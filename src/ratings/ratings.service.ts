...
export class RatingsService {
  ...
  async rate(input: RateInput) {
    ...
    try {
      ...
      // Rating.tripId and Rating.deliveryId are @unique — a second rating
      // attempt on the same trip/delivery hits that constraint. Turn Prisma's
      // raw P2002 into a clear 400 instead of leaking a database error.
      if (err?.code === "P2002") {
        throw new BadRequestException("This trip/delivery has already been rated");
      }
      throw err;
    }
